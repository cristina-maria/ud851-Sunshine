...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        final Long dateLong = mCursor.getLong(mCursor.getColumnIndex(
                WeatherContract.WeatherEntry.COLUMN_DATE));
        final String dateString = SunshineDateUtils.getFriendlyDateString(mContext, dateLong, true);

        final Integer weatherId = mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
        String weatherDescription = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);

        final Double minTemp = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        final Double maxTemp = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
        String minMaxTemp = SunshineWeatherUtils.formatHighLows(mContext, maxTemp, minTemp);

        String weatherSummary = dateString + " - " + weatherDescription + " - " + minMaxTemp;
